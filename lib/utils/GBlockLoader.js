...
function GBlockLoader () {
  ...
  this.parse = function (data, path, onLoad, onError) {
    ...
    Object.keys(json.shaders).forEach(function (key, i) {
      ...
      if (key.indexOf('fragment') > -1) json.shaders[key].uri = 'https://unboring.net/blocks-webAR/js/GBlockShaders/fragment-shader.glsl'
      else if (key.indexOf('vertex') > -1) json.shaders[key].uri = 'https://unboring.net/blocks-webAR/js/GBlockShaders/vector-shader.glsl'
    })
